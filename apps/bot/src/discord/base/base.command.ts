...
export async function baseCommandHandler(interaction: CommandInteraction){
    ...
    const cooldownTime = 30000; // Cooldown time in milliseconds (e.g., 5 seconds)
    ...
    if (userCooldowns.has(userId)) {
        ...
        if (timeLeft > 0) {
            await interaction.reply({
                content: `⏳ Porfavor espere ${Math.ceil(timeLeft / 1000)} segundos(s) antes de usar o comando novamente.`,
                ephemeral: true,
            });
            return;
        }
