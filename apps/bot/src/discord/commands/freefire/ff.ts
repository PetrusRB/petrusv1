...
export default createCommand({
    ...
    async run(interaction: ChatInputCommandInteraction): Promise<any> {
        ...
        const regiaoOption = interaction.options.getString("regiao")?.toLowerCase() ?? "br";

        const parsed = schema.safeParse({ jogador: jogadorOption });
        if (!parsed.success) {
            const err = parsed.error.errors.map(e => e.message).join(", ");
            await interaction.editReply({ content: `${ERROR_MESSAGES.VALIDATION_ERROR} ${err}` });
            return;
        }
        ...
        try {
            ...
            const clanLevelKey = `level${clan?.clanLevel || 0}` as keyof typeof settings.emojis.static;
            ...
            const embed = createEmbed({
                ...
                description: `${settings.emojis.static.freefire} Informações do jogador **${b.nickname}** na região **${b.region.toUpperCase()}**.`,

                fields: [
                    {
                        name: "📋 Informações da Conta",
                        value: `🆔 **ID:** ${b.accountId}\n 🪙 **Tenhe Passe** ${b.hasElitePass ? "Sim" : "Não"}\n 🔼 **Nível:** ${b.level}\n💎 **Curtidas:** ${b.liked}\n ✈️ **Rank no BR** ${b.rank + "/" + b.maxRank} \n 🔫 **Rank no CS** ${b.csRank + "/" + b.csMaxRank}\n`,
                        inline: true,
                    },
                    {
                        name: "💬 Social",
                        ...
                        )}\n🖋️ **Bio:** ${socialInfo?.signature || "_Sem biografia definida_"
                            }`,
                        inline: true,
                    },
                    ...(clan
                ...
                timestamp: new Date(),
                footer: {
                    text: `🌎 Região: ${b.region.toUpperCase()} • 🔖 Versão: ${b.releaseVersion ?? "Desconhecida"
                        }`,
                },
            });
