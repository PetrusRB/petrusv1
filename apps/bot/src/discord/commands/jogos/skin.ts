...
export default createCommand({
  ...
  async run(interaction): Promise<any> {
    ...
    if (!memberRoleId) {
      return interaction.editReply({
        content: `${settings.emojis.static.failed} Cargo de **membro** não configurado. Use \`/config cargos membro <id_do_cargo>\`.`,
      });
    }
    ...
    try {
      ...
      if (cacheEntry && cacheEntry.expiresAt > now) {
        ...
        // busca informações da skin
        const infoRes = await fetchSkinInfo(jogador);
        if (!infoRes.success) {
          // Se não foi encontrado.
          const msg = `Falha ao buscar informações da skin: ${infoRes.error}`;
          throw new Error(msg);
        }
        infoData = infoRes.data!;

        // tenta render; se falhar, fallback com skinRoute
        try {
