...
const supported_coins = ['bitcoin', 'ethereum'] as const;
  ...
  options: [
    {
      ...
      choices: supported_coins.map((coin) => ({ name: coin, value: coin })),
      required: true,
    },
    {
      name: 'moeda_local',
      ...
      type: ApplicationCommandOptionType.String,
      choices: SUPPORTED_CURRENCIES.map((curr) => ({
        name: curr,
        value: curr,
      })),
      required: false,
    ...
    try {
      ...
      if (!validated.success) {
        ...
        await interaction.editReply({
          content: `❌ Erro de validação: ${errorMessage}`,
        });
        return;
      }
      ...
      const apiUrl = `https://api.coingecko.com/api/v3/coins/${encodeURIComponent(
        moeda
      )}`;
      const resp = await fetch(apiUrl);
      if (!resp.ok) {
        await interaction.editReply({
          content: `❌ Não foi possível encontrar a moeda **${moeda}**.`,
        });
        return;
      }
      ...
      if (price === undefined) {
        await interaction.editReply({
          content: `❌ A moeda local "${moeda_local}" não é suportada.`,
        });
        return;
      }
      ...
      const embed = createEmbed({
        ...
        timestamp: new Date(),
        footer: {
          text: `Dados pela CoinGecko • ID: ${data.id}`,
          iconURL: image,
        },
      });
      ...
      await interaction.editReply({
        content: '❌ Ocorreu um erro ao consultar a criptomoeda.',
      });
    }
  },
});
