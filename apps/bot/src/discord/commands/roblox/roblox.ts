...
const schema = z.object({
    jogador: z.string({
        required_error: "A nome de usuário do jogador é obrigatória",
        invalid_type_error: "A nome de usuário do jogador deve ser uma string"
    })
});
...
export default createCommand({
    ...
    async run(interaction: ChatInputCommandInteraction): Promise<any> {
        ...
        try {
            ...
            const aggregated = await client.aggregateUserFull(lookup.id as number, { concurrency: 6 });
            ...
            const embed = createEmbed({
                ...
                fields: [
                    ...
                    { name: "📅 Criado em", value: (aggregated.details?.created ? new Date(aggregated.details.created).toLocaleString() : "—") as string, inline: true },
                    { name: `👥 Grupos (${aggregated.groups.length})`, value: (aggregated.groups.length ? aggregated.groups.slice(0, 20).map(g => `${g.group.name} (${g.role.name})`).join("\n") : "Nenhum") as string },
                ],
                footer: { text: `Roblox ID: ${lookup.id}` },
                timestamp: new Date()
            });

            await interaction.editReply({ embeds: [embed], content: SUCCESS_MESSAGES.SUCCESS_TITLE });
