...
export default createCommand({
  ...
  async run(interaction) {
    ...
    await interaction.reply({
      content: `${settings.emojis.anim.loading} Verificando possibilidades para desmutar...`,
      fetchReply: true,
      ephemeral: true,
    });
    if (
    ...
    if (targetMember.roles.cache.get(memberrole)) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Este membro já está desmutado.`,
      });
      return;
    }
