...
export default createCommand({
  ...
  options: [
    {
      name: 'alvo',
      description:
        'Alvo que vai ser expulso: /kick alvo: @alvo motivo: Motivo Aqui.',
  ...
  async run(interaction) {
    ...
    await interaction.reply({
      content: `${settings.emojis.anim.loading} Verificando Possibilidades de poder expulsar...`,
      fetchReply: true,
      ephemeral: true,
    });
    if (!botMe?.permissions.has(PermissionFlagsBits.KickMembers)) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Sem permissão para expulsar usuários!`,
      });
      return;
    }
    if (
    ...
    if (!targetMember?.kickable) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Eu não consigo expulsar este membro, pois ele não é kickavel, abortando...`,
      });
      return;
    }
    if (targetMember.id === user?.id) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Você não pode expulsar você mesmo.`,
      });
      return;
    }
    if (targetMember.id === botMe?.id) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Eu não posso me expulsar.`,
      });
      return;
    }
