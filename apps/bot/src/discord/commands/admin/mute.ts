...
export default createCommand({
  ...
  async run(interaction) {
    ...
    if (!muterole) {
      interaction.reply({
        content: `${settings.emojis.static.failed} O cargo de **mutado** não está configurado. Use /config categoria:cargos chave:mutado valor:id_do_cargo_aqui para configurar.`,
        ephemeral: true,
      });
      return;
    }
    ...
    await interaction.reply({
      content: `${settings.emojis.anim.loading} Verificando possibilidades para mutar...`,
      fetchReply: true,
      ephemeral: true,
    });
    if (
    ...
    if (targetMember.roles.cache.get(muterole)) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Este membro já está mutado.`,
      });
      return;
    }
    ...
    if (targetMember.id === user?.id) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Você não pode mutar você mesmo.`,
      });
      return;
    }
    if (targetMember.id === botMe?.id) {
      interaction.editReply({
        content: `${settings.emojis.static.failed} Eu não posso me mutar.`,
      });
      return;
    }
