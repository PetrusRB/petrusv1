...
export class RobloxClient {
  ...
  async lookupByUsername(username: string): Promise<UserLookup> {
    try {
      const json = await postJson<{ data?: any[] }>(
  ...
  async getUserGroups(userId: number, limit = 10): Promise<GroupRole[]> {
    try {
      const json = await getJson<{ data?: GroupRole[] }>(
        `${GROUPS_API}/users/${userId}/groups/roles`
      );
      return (json.data ?? []) as GroupRole[];
    } catch (err) {
      if (this.useNoblox && this.noblox?.getGroups) {
        const raw = (await this.noblox.getGroups(userId)) as any[];
        return raw.slice(0, limit).map((g: any) => ({
          group: { id: g.Id as number, name: g.Name as string },
          role: { id: g.RoleId as number, name: g.Role as string },
        })) as GroupRole[];
      }
      throw err;
    }
  ...
  async getFriends(userId: number, limit = 50): Promise<FriendEntry[]> {
    try {
      const json = await getJson<{ data?: any[] }>(
        `${FRIENDS_API}/users/${userId}/friends?limit=${limit}`
      );
  ...
  async getGamesFromUser(userId: number, limit = 10): Promise<GameInfo[]> {
    ...
    try {
      ...
      return games.slice(0, limit).map(
        (g) =>
          ({
  ...
  async searchCatalogItems(query: string, limit = 10): Promise<ItemSummary[]> {
    const url = `${CATALOG_API}/search/items/details?keyword=${encodeURIComponent(
      query
    )}&limit=${limit}`;
    try {
      ...
      return items.slice(0, limit).map((it) => ({
        ...
        creator: it.creator
          ? { id: it.creator.id as number, name: it.creator.name as string }
          : undefined,
      ...
      // legacy fallback
      try {
        const legacy = await getJson<any[]>(
          `https://search.roblox.com/catalog/json?Keyword=${encodeURIComponent(
            query
          )}&PageNumber=1&ResultsPerPage=${limit}`
        );
        return legacy.slice(0, limit).map((l) => ({
          id: l.Id as number,
  ...
  async getUserInventory(userId: number, limit = 50): Promise<ItemSummary[]> {
    try {
      ...
      const canView = await getJson<{ canView?: boolean }>(
        `${INVENTORY_API}/users/${userId}/can-view-inventory`
      ).catch(() => ({}));
      if (canView === false) return [];

      const inv = await getJson<any>(
        `${INVENTORY_API}/users/${userId}/assets?limit=${limit}`
      ).catch(() => null);
      const items = (inv?.data ?? inv?.items ?? []) as any[];
      return items.slice(0, limit).map((it) => ({
        id: (it.id ?? it.assetId) as number,
        name: (it.name ?? it.assetName) as string,
      })) as ItemSummary[];
    } catch (err) {
      ...
      if (this.useNoblox && this.noblox?.getInventory) {
        ...
        const inv = await (this.noblox.getInventory as any)(
          userId as number,
          0 as number,
          0 as number,
          limit as number
        );
        return (inv as any[]).slice(0, limit).map((i) => ({
          id: (i.assetId ?? i.id) as number,
  ...
  async aggregateUserFull(userId: number, opts?: { concurrency?: number }) {
    ...
    const results = await pMap<(typeof tasks)[0], any>(
      tasks,
      async (fn) => {
