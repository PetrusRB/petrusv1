...
export const generateSafeId = (prefix = 'id') => `${prefix}_${crypto.randomBytes(8).toString('hex')}`;
export const safeJSON = (data) => JSON.parse(JSON.stringify(data));
export function encryptString(plain, keyBase64) {
...
export function decryptString(cipherBase64, keyBase64) {
    const key = Buffer.from(keyBase64, 'base64');
